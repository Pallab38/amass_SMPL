...
### human_body_prior in "site-packages"
...
def extractData_createImg(body_data):
    ...
    body_dmpls = torch.tensor(body_data['dmpls'][frame_num:frame_num + 1]).float().to(device) ## torch.Size([1, 8])
    betas = torch.tensor(body_data['betas'][:10][np.newaxis]).float().to(device) # ## torch.Size([1, 10])
...
def showBodyMeshOnly(body_hand_mesh,ax):
    ...
    imw, imh = 1600, 1600
    mv = MeshViewer(width=imw, height=imh)
    # apply_mesh_tranfsormations_([body_hand_mesh],trimesh.transformations.rotation_matrix(90,(0,0,1)))
    # apply_mesh_tranfsormations_([body_hand_mesh], trimesh.transformations.rotation_matrix(30,(1,0,0)))
    # apply_mesh_tranfsormations_([body_hand_mesh], trimesh.transformations.rotation_matrix(30,(0,1,0)))
...
# def savePairPlots(dir_path,file_name):